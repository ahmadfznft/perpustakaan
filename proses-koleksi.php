...
if (isset($_POST['tambah'])) {
    ...
    if (mysqli_num_rows($result) > 0) {
        echo "<script>alert('Buku ini sudah ada dalam koleksi pribadi Anda!'); window.location.href='koleksi.php';</script>";
    } else {
        $sql = "INSERT INTO koleksipribadi (BukuID, UserID) VALUES ($buku, $id)";
        if (mysqli_query($conn, $sql)) {
            echo "<script>alert('Buku berhasil ditambahkan ke koleksi pribadi'); window.location.href='koleksi.php';</script>";
        } else {
...
if (isset($_GET['id'])) {
    ...
    if (mysqli_query($conn, $sql)) {
        echo "<script>alert('Buku berhasil dihapus dari koleksi pribadi'); window.location.href='koleksi.php';</script>";
    } else {
        echo "<script>alert('Gagal menghapus buku dari koleksi pribadi'); window.location.href='koleksi.php';</script>";
    }
}
